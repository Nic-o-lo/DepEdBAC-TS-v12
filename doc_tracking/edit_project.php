...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_stage'])) {
    ...
    if ($isSubmittedVal == 1) { // Only validate on "Submit" action
        ...
        // This server-side validation is a fallback; client-side HTML 'required' handles immediate feedback
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        stageForms.forEach(form => {
            form.addEventListener('submit', function(event) {
                ...
                const officeField = form.querySelector(`input[name="office_${safeStage}"]`);
                const remarkField = form.querySelector('input[name="remark"]');
                const submitButton = form.querySelector('button[name="submit_stage"]');

                // Only validate if the button pressed is "Submit" (not "Unsubmit")
                // And if the button is not disabled
                if (submitButton && submitButton.textContent === 'Submit' && !submitButton.disabled) {
                    let fields = [];

                    // 'Created' is required if editable by admin and not Purchase Request
                    if (createdField && !createdField.disabled && stageName !== 'Purchase Request') {
                        fields.push({ element: createdField, name: 'Created' });
                    }
                    fields.push({ element: approvedAtField, name: 'Approved' });
                    fields.push({ element: officeField, name: 'Office' });
                    fields.push({ element: remarkField, name: 'Remark' });
                    ...
                    for (const field of fields) {
                        if (field.element && field.element.value.trim() === '') {
                            ...
                            // Add a visual cue or specific error handling here if desired
                            // For now, the browser's default validation message will appear if 'required' is set
                        }
                    }
                    ...
                    if (!allFieldsFilled) {
                        ...
                        if (firstEmptyField) {
                            ...
                            // Optionally, trigger a custom validation message here
                            // For a native-like popup, the 'required' attribute is best.
                            // If you want a custom overlay, you'd implement it here.
                        }
                        alert("Please fill out all required fields: Created, Approved, Office, and Remark."); // A simple alert for now
                        // For a more advanced popup like the image, you'd need a custom modal/tooltip library
                    }
                }
            });
        });
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="project-header">
            ...
            <?php if ($isAdmin): ?> <form action="edit_project.php?projectID=<?php echo $projectID; ?>" method="post" style="margin-bottom:10px;">
            ...
            <?php if ($isAdmin): ?> <textarea name="projectDetails" id="projectDetails" required><?php echo htmlspecialchars($project['projectDetails']); ?></textarea>
            <?php else: ?>
            ...
            <?php if ($isAdmin): ?> <button type="submit" name="update_project_header" class="update-project-details-btn">
    ...
    <form action="edit_project.php?projectID=<?php echo $projectID; ?>" method="post" class="stage-form">
        <div class="stage-card">
            ...
            <label>Office:</label>
            <input type="text" name="office_<?php echo $safeStage; ?>" value="<?php echo $value_office; ?>"
                <?php if ($disableFields) echo "disabled"; ?>
                <?php if (!$disableFields) echo "required"; ?>>
